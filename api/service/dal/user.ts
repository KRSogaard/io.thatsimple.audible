...
export class UserService {
  ...
  constructor() {
    this.logger = new APILogger('UserService');
  }

  async addBookToUser(bookId: number, userId: number): Promise<any> {
    this.logger.trace('Adding book ' + bookId + ' to user ' + userId);
    ...
    if (!results || results.length === 0) {
      ...
      this.logger.trace('Book ' + bookId + ' already in user ' + userId + ' library');
    }
  }

  async createUser(user: RegisterUser): Promise<number> {
    this.logger.trace('Creating user: ' + user.username);
  ...
  async verifyUser(email: string, password: string): Promise<TokenResponse> {
    this.logger.trace('Verifying user email: ' + email);
  ...
  async getUserByToken(token: string): Promise<UserWithPassword> {
    ...
    this.logger.trace('Getting user by token: ' + token);
  ...
  async getUserById(userId: number): Promise<UserWithPassword> {
    this.logger.trace('Getting user by id: ' + userId);
  ...
  async getUserByName(username: string): Promise<UserWithPassword> {
    this.logger.trace('Getting user by name: ' + username);
  ...
  async getUserByEmail(email: string): Promise<UserWithPassword> {
    this.logger.trace('Getting user by email: ' + email);
  ...
  async getMyBooks(userId: number): Promise<number[]> {
    this.logger.trace('Getting books for user: ' + userId);
  ...
  async archiveSeries(userId: number, seriesId: number): Promise<void> {
    this.logger.trace('Archiving series ' + seriesId + ' for user ' + userId);
  ...
  async unarchiveSeries(userId: number, seriesId: number): Promise<void> {
    this.logger.trace('Unarchiving series ' + seriesId + ' for user ' + userId);
  ...
  async getArchivedSeries(userId: number): Promise<number[]> {
    this.logger.trace('Getting archivied series for user ' + userId);
  ...
  async createJob(userId: number, type: string, payload: string): Promise<number> {
    this.logger.trace('Creating job for user ' + userId);
  ...
  async finishJob(jobId: number): Promise<number> {
    this.logger.trace('Finishing job with id ' + jobId);
  ...
  async getCurrentJobs(userId: number): Promise<Job[]> {
    this.logger.trace('Getting current jobs for user ' + userId);
